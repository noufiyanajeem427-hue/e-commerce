...
function InvoiceQueryContent() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      toast.error("Error creating PDF.", { id: "pdf-gen-query" });
    } finally {
  ...
  if (!order) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <h1 className="text-3xl font-black text-white mb-2">No Order Found</h1>
        <p className="text-zinc-400 text-sm max-w-md mx-auto mb-8">
          Please select an order from your order history to view its tax invoice.
        </p>
        <Link
          href="/orders"
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
      ...
      <div className="flex justify-center pb-12">
        <InvoiceView order={order} id={`query-invoice-${order.id}`} />
      </div>
    </div>
  );
}
